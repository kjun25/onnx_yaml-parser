...
def extract_model(onnx_path, yaml_dir_path, save_dir_path):
    ...
    for file in file_list_yaml:
        with open(yaml_dir_path + file, "r") as stream:
            ...
            print(input_names)
            print(output_names)
            onnx.utils.extract_model(onnx_path, save_dir_path + file.split(".yaml")[0] + '.onnx', input_names,
                                     output_names)
...
def main(args):
    ...
    print(np.count_nonzero((np.abs(res1[0] - res2[0]) > 0.00001).astype(int)))
